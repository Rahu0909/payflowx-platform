...
public interface MerchantWebhookRepository extends JpaRepository<MerchantWebhook, UUID> {
    ...
    List<MerchantWebhook> findByMerchantIdAndDeletedFalse(UUID merchantId);

    Optional<MerchantWebhook> findByIdAndDeletedFalse(UUID webhookId);

    long countByMerchantIdAndDeletedFalse(UUID merchantId);

    Optional<MerchantWebhook> findFirstByMerchantIdAndDeletedFalseAndStatus(UUID merchantId, WebhookStatus status);
}
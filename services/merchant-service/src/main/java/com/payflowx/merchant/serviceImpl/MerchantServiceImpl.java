...
public class MerchantServiceImpl implements MerchantService {
    ...
    public MerchantResponse createMerchant(UUID authUserId, CreateMerchantRequest request) {
        ...
        Merchant merchant = Merchant.builder().authUserId(authUserId).businessName(request.businessName()).email(request.email()).phone(request.phone()).category(request.category()).status(MerchantStatus.SUSPENDED).statusChangedAt(LocalDateTime.now()).statusReason("INITIAL_CREATION").build();
    ...
    public MerchantWebhookResponse getMerchantWebhook(String merchantId) {
        ...
        MerchantWebhook webhook = merchantWebhookRepository.findFirstByMerchantIdAndDeletedFalseAndStatus(merchantUuid, WebhookStatus.ACTIVE).orElseThrow(() -> new BusinessValidationException(ErrorCode.WEBHOOK_NOT_FOUND));
        return MerchantWebhookResponse.builder().merchantId(merchantId).webhookUrl(webhook.getWebhookUrl()).webhookSecret(webhook.getWebhookSecret()).webhookEnabled(true).build();
    }
}
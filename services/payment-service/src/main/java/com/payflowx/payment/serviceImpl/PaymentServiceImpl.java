...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentResponse createPayment(CreatePaymentRequest request, String idempotencyKey) {
         ...
         * CREATE PAYMENT
         */
        Payment payment = Payment.builder().paymentReference(ReferenceGeneratorUtil.generatePaymentReference()).orderId(request.orderId()).merchantId(order.merchantId()).userId(userId).amount(request.amount()).currency(Currency.valueOf(order.currency())).paymentMethod(request.paymentMethod()).description(request.description()).status(PaymentStatus.CREATED).build();
